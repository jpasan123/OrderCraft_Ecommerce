(function ($) {
    ...
    $('#selection').parent().append('<ul class="list-item" id="newselection" name="selection"></ul>');
    $('#selection option').each(function(){
        $('#newselection').append('<li value="' + $(this).val() + '">'+$(this).text()+'</li>');
    });
    $('#selection').remove();
    $('#newselection').attr('id', 'selection');
    $('#selection li').first().addClass('init');
    $("#selection").on("click", ".init", function() {
        $(this).closest("#selection").children('li:not(.init)').toggle('slow');
    });
    ...
    var allOptions = $("#selection").children('li:not(.init)');
    $("#selection").on("click", "li:not(.init)", function() {
        allOptions.removeClass('selected');
        $(this).addClass('selected');
        $("#selection").children('.init').html($(this).html());
